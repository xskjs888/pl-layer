...
const ready = {
  ...
  record(layero) {
    let area = [layero.width(), layero.height(), layero.position().top, layero.position().left + parseFloat(layero.css('margin-left'))]
...
const layer = {
  ...
  confirm(content, options, yes, cancel) {
    ...
    return this.open(Object.assign({}, { content, btn: ready.btn, yes, btn2: cancel }, type ? {} : options))
  },
...
class Container {
  ...
  create() {
    ...
    config.type == 2 && layer.ie == 6 && this.layero.find('iframe').attr('src', content[0])
  ...
  offset() {
    ...
    if (!config.fixed) {
      this.offsetTop = /%$/.test(this.offsetTop) ? (win.height() * parseFloat(this.offsetTop)) / 100 : parseFloat(this.offsetTop)
      this.offsetLeft = /%$/.test(this.offsetLeft) ? (win.width() * parseFloat(this.offsetLeft)) / 100 : parseFloat(this.offsetLeft)
    ...
    layero
      ...
      .on('click', function() {
        ...
        if (index === 0) {
          ...
          let close = config['btn' + (index + 1)] && config['btn' + (index + 1)](that.index, layero)
...
layer.style = function(index, options, limit) {
  ...
  if (type === ready.type[2]) {
    ...
    contElem.css({
      height:
        parseFloat(options.height) - titHeight - btnHeight - parseFloat(contElem.css('padding-top')) - parseFloat(contElem.css('padding-bottom'))
    })
  }
}
...
layer.prompt = function(options, yes) {
  ...
  let prompt,
    content =
      options.formType == 2
        ? '<textarea class="layui-layer-input"' + style + '>' + (options.value || '') + '</textarea>'
        : (function() {
            return (
              '<input type="' + (options.formType == 1 ? 'password' : 'text') + '" class="layui-layer-input" value="' + (options.value || '') + '">'
            )
          })()
...
layer.photos = function(options, loop, key) {
  ...
  let photos = type ? options.photos : {},
    data = photos.data || []
